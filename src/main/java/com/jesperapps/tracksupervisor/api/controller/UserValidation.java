...
//		return new ResponseEntity(response2, HttpStatus.CONFLICT);
	...
	public ResponseEntity loginuserbyForgotPassword(@RequestBody UserRequestEntity adminRequest) {

		User emailFromDb=userService.findUserByEmailOrPhoneNumber(adminRequest.getEmail(),adminRequest.getPhoneNumber());
		if(emailFromDb!=null) {
		
			if(userService.checkOtpIsSame(adminRequest.getOtp(), emailFromDb.getOtp())) {
				if(emailFromDb.getVerificationStatus()==1) {
					User userFromDb=userService.findByEmailAndOtp(emailFromDb.getEmail(),emailFromDb.getOtp());
//		     		System.out.println(userFromDb);	
//		      		System.out.println("Login Successfull");
					Organization orgfromdb=userFromDb.getOrganization();
					if(orgfromdb == null ) {
...
//				System.out.println("Password Invalid");	
				UserResponseEntity response1=new UserResponseEntity();
				response1.setStatusCode(409);
				response1.setDescription("Otp Invalid");
				return new ResponseEntity(response1, HttpStatus.CONFLICT);
